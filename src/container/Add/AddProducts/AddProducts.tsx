...
function AddProducts({ sectionId, subSectionId }: AddProducts) {
  ...
  const submitLink = async () => {
    try {
      ...
      const type = linkInputs?.trim()?.toLowerCase()?.includes("amazon")
        ? "AMAZON"
        : linkInputs?.trim()?.toLowerCase()?.includes("flipkart")
        ? "FLIPKART"
        : "EXTERNAL";
      setLinkType(type);
  ...
  const createLinkComponent = async () => {
    ...
    try {
      ...
      showToast("Shopping link added successfully!", "success");
  ...
  return (
    <>
      <div className={`${styles.addProductContainer} pageWidth`}>
        <Header title="Add Shopping Link" />
...
export default transition(AddProducts);
